...
Deno.serve(async (req) => {
  try {
    const { slug,page_data } = await req.json();

    if (!slug || !page_data) {
      return new Response(JSON.stringify({ error: "Missing slug or page_data" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    
    const { data:get_data, error:get_err } = await supabase.from("Page").select("*").match({"slug":slug});
    ...
    if(!get_data || get_data.length === 0){
      ...
      const { data, error } = await supabase.from("Page").update(
        {data:page_data}
      ).eq("slug", slug).select("*");

...
Deno.serve(async (req) => {
  try {
    ...
    if (!slug || !curriculum_data) {
      return new Response(JSON.stringify({ error: "Missing slug or curriculum_data" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    
    const { data:get_data, error:get_err } = await supabase.from("Curriculum").select("*").match({"slug":slug});

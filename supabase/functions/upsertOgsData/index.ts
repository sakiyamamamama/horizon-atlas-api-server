...
Deno.serve(async (req) => {
  try {
    ...
    const { data:get_data, error:get_err } = await supabase.from("OgsData").select("*").match({"blockId":blockId});
    ...
    if(!get_data || get_data.length === 0){
      const { data, error } = await supabase.from("OgsData").insert([
      ...
      const { data, error } = await supabase.from("OgsData").update(
        {data:ogs_data}
